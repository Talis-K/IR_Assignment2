...
class Environment:
    def __init__(self):
        self.built = 0 #Terminal Print Counter
        #Ground Plane
        ...
        self.env._parent = self # Allows RobotUnit to access Environment
    ...
    def load_robots(self):
        ...
        self.kr6 = RobotUnit(KR6(), self.env, SE3(0.7, 1.0, self.ground_height),                                                        collision_zones=kr6_zones) #KR6
        self.lbr = RobotUnit(LBR(), self.env, SE3(0.7, 0.0, self.ground_height),                                                        collision_zones=lbr_zones) #LBR
        self.lwr = RobotUnit(LWR(), self.env, SE3(0.7, -1.0, self.ground_height),                                                       collision_zones=lwr_zones) #LWR
        self.ur3 = RobotUnit(UR3(), self.env, SE3(-0.7, 0.0, self.ground_height + self.ur3_stand), q_init=[pi/2, -pi/2, 0, 0, pi/2, 0], collision_zones=ur3_zones) #UR3 
    ...
    def load_conveyer(self):
        ...
        top      = Cuboid(scale = [0.3, 2.5, self.conveyer_height],pose=SE3(0, 0, self.conveyer_height / 2 + self.ground_height + self.support_height),color=[0.1, 0, 0])
    ...
    def translate_object(self, target_obj, target_pose):
        if isinstance(target_obj, tuple):
            idx, mesh = target_obj
            initial_pose = SE3(mesh.T)
            print(f"Translating brick {idx} from: \n{initial_pose} To: \n{target_pose}")
        for angle in np.linspace(0, 1, 50):
            mesh.T = initial_pose.interp(target_pose*SE3.Ry(pi), angle).A #Creating the path for thr brick to travel from intial to goal pose
